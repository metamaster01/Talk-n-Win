...
export async function POST(req: Request) {
  try {
    ...
    const mode = (body.mode ?? "single") as "single" | "cart" | "demo";
    const {
      courseId,
      userId,
      demoClassId,
    ...
    // For normal course checkout we STILL require userId
    if ((mode === "single" || mode === "cart") && !userId) {
      return NextResponse.json(
        { error: "userId is required for this checkout mode" },
        { status: 400 }
      );
    }
    ...
    if (mode === "demo") {
      ...
      amountInPaise = Math.round(Number(demoClass.price) * 100);
      description = `Demo Class: ${demoClass.title}`;
      receipt = `demo_${demoClass.id.slice(0, 8)}_${Date.now()
        .toString()
        .slice(-6)}`;
    } else if (mode === "single") {
      if (!courseId) {
